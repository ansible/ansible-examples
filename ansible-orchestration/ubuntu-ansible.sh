#!/usr/bin/env bash
# install ansible for a Ubuntu server
...
# append ansible-head key on the remote hosts: 
#for h in $hosts do 
#    ssh-copy-id -i ~/.ssh/id_rsa.pub vagrant@$h
#done
# send the public key to the target servers
# problem: this still asks for a password
# cat .ssh/id_rsa.pub | ssh vagrant@192.168.35.30 'cat >> .ssh/authorized_keys'
